...
public class PaxWebChromeClient extends WebChromeClient {
    ...
    private static final int CHOOSE_REQUEST_CODE = 9001;
...
//        Log.i("TAG", "onShowFileChooser-onActivityResult5: " + GetPathFromUri4kitkat.getPath(mActivity, Uri.parse(data.getDataString())));
        if (resultCode == Activity.RESULT_OK) {
            switch (requestCode) {
                case CHOOSE_REQUEST_CODE:
                    ...
                    if (null != uploadFiles) {
                        Uri result = Uri.fromFile(new File(GetPathFromUri4kitkat.getPath(mActivity, Uri.parse(data.getDataString()))));
