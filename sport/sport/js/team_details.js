...
function getSchedule(){
    $.ajax({
        ...
        dataType: "json",
        data: {match_id: "1",match_season:"2017-2018"},
        // data: {match_id: param.match_id},
...
function showDataTable(index) {
    ...
    for (let i = 0; i< result.rounds.length; i++) {//然后循环，填充数据
        if(parseInt(result.rounds[i]) === parseInt(index) ){
            ...
            // console.log("111",result.rounds[i]);
        }
    }
    let template=document.getElementById('template-super-league-table-list').innerHTML;
    //传递一个有数据的数组进去
    document.getElementById('super-league-table').innerHTML = doT.template( template )( data );
}
//杯赛数据统计
function getCupData(){
    $.ajax({
